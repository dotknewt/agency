...
`instructions/branch-hygiene.md` — one branch per PR, cleanup after squash-merge, stale-branch sweep.
`instructions/commit-vs-pr.md` — when to commit directly vs. open a PR. Do not restate these; they are already in context.
...
See `instructions/branch-hygiene.md` for local and remote branch cleanup after recovery.
